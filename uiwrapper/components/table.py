...
class Table(ContainerAction):
    COLS = ' [data-test="cell"][data-column="{}"]'
    COLS_NO = " td:nth-child({})"
    ALERT_KEY = ' [data-test="alert-icon"]'
    EXPANDED_ROW = ' [data-expansion-row="true"] [data-test="cell"] dl[data-test="definition-list"]'
    ...
    def __init__(self, driver, container: dict) -> None:
        ...
        new_locator = {
            "input_number": [None, self.value + " .inputNumber"],
            "status_column": [None, ' [data-test="status"]'],
            "clear_filter": [None, ' [data-test="clear"]'],
            "search_box": [None, ' [data-test="textbox"][placeholder="filter"]'],
            "loader": [None, self.value + ' [data-test="wait-spinner"]'],
            "popup": [None, " div.deletePrompt"],
            "clone_btn": [None, " button.cloneBtn"],
            "edit_btn": [None, self.value + " button.editBtn"],
            "delete_btn": [None, self.value + " button.deleteBtn"],
            "delete_prompt": [None, '[data-test="modal"] div.deletePrompt p'],
            "delete_cancel": [None, '[data-test="button"][label="Cancel"]'],
            "delete_close": [None, '[data-test="close"]'],
            "status_button": [None, ' [data-test="button"][role="switch"]'],
            "switch_to_page": [None, self.value + " button[data-test-page]"],
            "main_table": [None, 'table[data-test="main-table"]'],
            "table_head": [None, ' [data-test="head"]'],
            "table_headers": [None, self.value + ' th[data-test="head-cell"]'],
            "table_container": [None, self.value + ' [data-test="table"]'],
            "expand": [None, '[data-test="expand"]'],
            "rows": [None, self.value + ' tbody[data-test="body"] tr[data-test="row"]'],
            "expanded_row": [None, self.EXPANDED_ROW],
            "expanded_row_term": [None, self.EXPANDED_ROW + ' dt[data-test="term"]'],
            "expanded_row_desc": [
                None,
                self.EXPANDED_ROW + ' dd[data-test="description"]',
            ],
        }
        self.locator.update_locaters(new_locator)
        self.toast = Toast(self.driver, self.name)
    ...
    def get_list_of_rows(self, rows: Optional[list] = None) -> list:
        ...
        LOGGER.info("ROWS: {}, COUNT: {}".format(rows, len(rows)))
        return rows
    ...
    def delete_row(self, input_name: str, action: Optional[str] = None):
        ...
        try:
            ...
            if action == "delete":
                ...
                LOGGER.debug(
                    "Opening the delete popup to read the conformation message."
                )
                prompt_msg = self.get_element(
                    *self.locator.get_locator("delete_prompt")
                ).text
                return prompt_msg
    ...
    def search(self, search: str):
        ...
        """
        LOGGER.info("Searching in table, search: {}".format(search))
        self.wait_for_element("table_container")
        try:
            self.enter_text("search_box", search)
        except:
            search_locator = {"search_box": [None, '[data-test="textbox"][placeholder="Search..."]']}
            self.locator.update_locaters(search_locator)
